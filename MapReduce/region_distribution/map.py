...
for line in sys.stdin:
    ...
    try:
        ...
        pickup_year = datetime.strptime(pickup_datetime, '%Y-%m-%d %H:%M:%S').year
        pickup_month = datetime.strptime(pickup_datetime, '%Y-%m-%d %H:%M:%S').month
        pickup_day = datetime.strptime(pickup_datetime, '%Y-%m-%d %H:%M:%S').day
        pickup_hour = datetime.strptime(pickup_datetime, '%Y-%m-%d %H:%M:%S').hour
        pickup_weekday = date(pickup_year, pickup_month, pickup_day).isoweekday()

        dropoff_year = datetime.strptime(dropoff_datetime, '%Y-%m-%d %H:%M:%S').year
        dropoff_month = datetime.strptime(dropoff_datetime, '%Y-%m-%d %H:%M:%S').month
        dropoff_day = datetime.strptime(dropoff_datetime, '%Y-%m-%d %H:%M:%S').day
        dropoff_hour = datetime.strptime(dropoff_datetime, '%Y-%m-%d %H:%M:%S').hour
        ...
        if pickupRegion is not None:
            print "%s,%d,%d,%d,%d,%d\t%s\t%s" %(pickupRegion, pickup_year, pickup_month, pickup_day, pickup_hour, pickup_weekday, pickup_longitude, pickup_latitude)
        
        # if dropoffRegion is not None:
        #     print "%s,%d,%d,%d,%d,%d\t%s\t%s" %(dropoffRegion, dropoff_year, dropoff_month, dropoff_day, dropoff_hour, dropoff_weekday, dropoff_longitude, dropoff_latitude)
    except:
        print "Program fails due to the exception: ", sys.exc_info()[0]
